...
$(function () {
    $('.lk-header .center-page').addClass('on');
    ...
    $('.user-right .wait-record button').on('click', function () {
        ...
        $.ajax({
            url: '/api/user/houseSubscribe/date',
            type: 'POST',
    ...
    $('.user-right .subscribe button').on('click', function () {
        ...
        layer.confirm('确认要取消预约吗', {
            ...
            $.ajax({
                url: '/api/user/houseSubscribe?houseId=' + selected,
                type: 'DELETE',
                success: function (data) {
                    if (data.code === 200) {
    ...
    $('.edit-info .nameSubmit').on('click', function () {
        var name = $('.modify-tab input[name="username"]').val();
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '.wait-record .content',
                ...
                $.get('/api/user/houseSubscribe/list?status=1' + '&start=' + start + '&size=3',
                    function (res) {
                        layui.each(res.data, function (index, tuple) {
                            var house = tuple;
                            var content = '<li><input type="radio" name="houseId" value="' + house.houseId + '">预约此房源'
                                + '<div class="cover fl">'
                                + '<img src="http://qf3t3aofm.hn-bkt.clouddn.com/' + house.cover + '" width="100px" height="80px"></div> ' +
...
function loadSubscribeList() {
    layui.use('flow', function () {
        ...
        var flow = layui.flow;
        flow.load({
            elem: '#subscribe-record-list', //指定列表容器
            scrollElem: '.subscribe .content',
                ...
                $.get('/api/user/houseSubscribe/list?status=2' + '&start=' + start + '&size=3',
                    function (res) {
                        layui.each(res.data, function (index, tuple) {
                            var house = tuple;
                            var content = '<li><span' +
                                ' class="order-time">预约时间：' + (new Date(house.orderTime)).Format("yyyy-MM-dd")
                                ...
                                + '<i></i><span>' + house.floor + '层(共' + house.totalFloor + '层)</span><span>' +
                                house.buildYear + '建</span><span>-</span><span>' + house.street + '</span><span>' +
                                house.price + '元/月</span></div></div><div class="fl"><input type="radio" name="houseId" value="' + house.houseId + '">取消此预约</div></li>';
                            lis.push(content);
                        });
...
function loadFinishList() {
    layui.use('flow', function () {
        ...
        var flow = layui.flow;
        flow.load({
            elem: '#finish-record-list', //指定列表容器
            scrollElem: '.finish-record .content',
                ...
                $.get('/api/user/houseSubscribe/list?status=3' + '&start=' + start + '&size=3',
                    function (res) {
                        layui.each(res.data, function (index, tuple) {
                            var house = tuple;
                            var content = '<li><span' +
                                ' class="order-time">看房时间：' + (new Date(house.orderTime)).Format("yyyy-MM-dd")
                                + '</span><div class="cover fl">'
                                + '<img src="http://qf3t3aofm.hn-bkt.clouddn.com/' + house.cover + '" width="100px" height="80px"></div> ' +
                                '<div class="info fl"><a><h1>' + house.title + '</h1></a><div class="des1">' +
                                '<i></i><span><a href="#" target="_blank">' + house.district + '</a></span>' +
