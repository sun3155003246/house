...
var table = $('#data-table').DataTable({
        ...
        columnDefs: [{ // 定义表格样式
            ...
            render: function (data, type, row, meta) {
                return '<td class="text-l"><u style="cursor:pointer" class="text-primary"' +
                    'onClick="house_edit(\'查看\', \'/houseShow?id=' + row.houseSubscribeId + '\')" title="查看">' + data + '</u></td>';
            }
        }, {
            targets: 2,
            render: function (data, type, row, meta) {
                return '<td><img onClick="house_edit(\'查看\', \'/houseShow?id=' + row.houseSubscribeId + '\')" title="查看"' +
            ...
            render: function (data, type, row, meta) {
                return '<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5"' +
                    ' onClick="finishSubscribe(this,' + row.houseId + ')" href="javascript:;"' +
                    ' title="带看完成"><i class="Hui-iconfont">&#xe603;</i></a></td>';
            }
        }],
        ajax: {
            type: "GET",
            url: "/admin/houseSubscribe/list", // 服务器url
            cache: false,
            data: function (data) {
                var params = {};
...
function userTip(userId) {
    $.get('/admin/user/' + userId, function (res) {
        layer.open({
            ...
            ' font-weight: 300;">用户名：' + res.data.username + '<br>联系电话：' + res.data.phone + '</div>'
        });
    });
}
