    ...
    columnDefs: [{ // 定义表格样式
        ...
        render: function (data, type, row, meta) {
            return '<td class="text-l"><u style="cursor:pointer" class="text-primary"' +
                'onClick="house_edit(\'查看\', \'/admin/houseShow?id=' + data + '\')" title="查看">' + data + '</u></td>';
        }
    }, {
        targets: 1,
        render: function (data, type, row, meta) {
            return '<td class="text-l"><u style="cursor:pointer" class="text-primary"' +
                'onClick="house_edit(\'查看\', \'/admin/houseShow?id=' + row.houseId + '\')" title="查看">' + data + '</u></td>';
        }
    }, {
        targets: 2,
        render: function (data, type, row, meta) {
            return '<td><img onClick="house_edit(\'查看\', \'/admin/houseShow?id=' + row.houseId + '\')" title="查看"' +
                ' class="picture-thumb" src="http://qf3t3aofm.hn-bkt.clouddn.com/' + data + '?imageView2/1/w/200/h/100"></td>';
        }
    }, {
        targets: 7,
        render: function (data, type, row, meta) {
        ...
        render: function (data, type, row, meta) {
            var prefix = '<td class="f-14 td-manage">',
                ...
                suffix = '<a style="text-decoration:none" class="ml-5"' +
                    ' onClick="house_edit(\'房源编辑\', \'/admin/houseEdit?id=' + row.houseId + '\')" href="javascript:;"' +
                    ' title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5"' +
                    ' onClick="house_del(this, ' + row.houseId + ')" href="javascript:;" title="删除"><i' +
                    ' class="Hui-iconfont">&#xe6e2;</i></a></td>';
            if (data_status === 0) { // 待审核
                content = '<a style="text-decoration:none" onClick="house_pass(this,' + row.houseId + ')"' +
                    ' href="javascript:;" title="发布">发布</a>&nbsp;';
            } else if (data_status === 1) { // 已发布
                content = '<a style="text-decoration:none" onClick="house_stop(this,' + row.houseId + ')"' +
